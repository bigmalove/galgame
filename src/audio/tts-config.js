...
function _normalizeGptSoVitsModel(model, globalDefaults = {}) {
  ...
  const defaultRefPath = _normalizePathSep(paths.defaultRefAudioPath);
  let defaultRef = null;

  // Path wins when present to match UI editing expectations.
  if (defaultRefPath) {
    defaultRef = refAudios.find(item => item.path === defaultRefPath) || null;
  }
  if (!defaultRef && defaultRefId) {
    defaultRef = refAudios.find(item => item.id === defaultRefId) || null;
  }
  if (!defaultRef && refAudios.length > 0) {
    defaultRef = refAudios[0];
  }

  if (defaultRef) {
    defaultRefId = defaultRef.id;
    paths.defaultRefAudioPath = defaultRef.path;
  } else {
    defaultRefId = '';
    paths.defaultRefAudioPath = defaultRefPath;
  }
...
function _modelToVoice(model, globalDefaults = {}) {
  ...
  const defaultRefPath = String(normalized.paths?.defaultRefAudioPath || '').trim();
  const defaultRef =
  ...
  const refAudioPath = String(defaultRef?.path || defaultRefPath || '').trim();
...
function _getGptSoVitsVoiceSourceList(cfg) {
  ...
  // If model manager data exists, use it as the single source of truth.
  // This prevents stale legacy `voices` entries (often old absolute paths)
  // from being picked during playback.
  if (modelVoices.length > 0) return modelVoices;
...
export function mergeAutoImportedGptSoVitsModels(currentModels, discoveredModels, cfg = getGptSoVitsConfig()) {
  ...
  _safeArray(discoveredModels).forEach(item => {
    ...
    if (!incoming) return;
    const incomingName = _normalizeModelMatchKey(incoming.name);
    const incomingId = _normalizeModelMatchKey(incoming.id);
    if (!incomingName && !incomingId) return;

    const index = next.findIndex(model => {
      const name = _normalizeModelMatchKey(model?.name);
      const id = _normalizeModelMatchKey(model?.id);
      if (incomingName && name === incomingName) return true;
      if (incomingId && id === incomingId) return true;
      return false;
    });
    if (index < 0) {
      next.push(incoming);
    ...
    // Import wins: replace whole model payload (paths/refs/defaultRef/params).
    next[index] = incoming;
    updated += 1;
  });
