...
export function buildImageGenConfigPane(settings) {
  ...
  return `
    <div class="gal-bg-source-selector" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: rgba(15,23,42,0.08); border: 1px solid rgba(71,85,105,0.35); border-radius: 10px; margin-bottom: 10px; flex-wrap: wrap;">
      ...
      <label class="gal-radio-label" style="cursor:pointer;display:flex;align-items:center;gap:4px;color:${src === 'chatu8' ? BG_SOURCE_COLORS.chatu8 : BG_SOURCE_INACTIVE_COLOR};font-size:0.85rem;font-weight:600;"><input type="radio" name="gal-bg-source" value="chatu8" ${src === 'chatu8' ? 'checked' : ''} style="accent-color:${BG_SOURCE_COLORS.chatu8};"> 智绘姬</label>
    ...
    <div class="gal-chatu8-source-note" style="display:${src === 'chatu8' ? 'block' : 'none'}; padding: 10px 12px; margin-bottom: 10px; border-radius: 8px; background: rgba(15,118,110,0.12); border: 1px solid rgba(15,118,110,0.35); color: #0f766e; font-size: 0.85rem; font-weight: 600;">智绘姬模式已开启：本插件不会触发 ComfyUI / 大香蕉 / NovelAI / Wallhaven 背景生图，只识别智绘姬(st-chatu8)在消息中渲染出的图片。</div>
    <div class="gal-imagegen-cgmode-row" style="display:${src === 'chatu8' ? 'none' : 'flex'}; justify-content: space-between; align-items: center; gap: 12px; padding: 10px 16px; background: rgba(15,23,42,0.08); border: 1px solid rgba(71,85,105,0.35); border-radius: 10px; margin-bottom: 10px;">
      <div><label style="font-weight: 700; color: #1f2937; font-size: 0.9rem;"><i class="fa-solid fa-user-astronaut" style="margin-right: 6px;"></i>生成剧情CG</label><div style="font-size: 0.75rem; color: #64748b;">开启：生成包含人物的剧情CG | 关闭：生成纯场景背景。对 ComfyUI / 大香蕉 / NovelAI / Wallhaven 通用（Wallhaven 下表现为允许人物关键词）；智绘姬不受此开关影响。</div></div>
      <label class="gal-realtime-switch"><input type="checkbox" id="gal-imagegen-cgmode" ${settings.imageGenCgMode ? 'checked' : ''}><span class="gal-realtime-slider"></span></label>
    </div>
...
function buildWallhavenPane(settings) {
  ...
  <div style="padding: 15px; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius: 10px; border: 1px solid #0f3460; margin-top: 8px;">
    ...
    <div style="margin-bottom: 12px; font-size: 0.75rem; color: #8892b0;"><i class="fa-solid fa-circle-info"></i> 人物关键词开关已移至顶部通用「生成剧情CG」开关（开启后允许人物关键词）。</div>
...
export function bindImageGenConfigEvents($container, settings) {
  ...
  $container.find('input[name="gal-bg-source"]').on('change', async function () {
    ...
    // 通用「生成剧情CG」开关对智绘姬无效，选中智绘姬时隐藏
    $container.find('.gal-imagegen-cgmode-row').css('display', value === 'chatu8' ? 'none' : 'flex');
    ...
    const names = { none: '关闭', chatu8: '智绘姬', comfyui: 'ComfyUI', banana: '大香蕉', novelai: 'NovelAI', wallhaven: 'Wallhaven' };
    showToast(`背景图来源: ${names[value] || value}`);
  });

  // 通用「生成剧情CG」开关（comfyui/banana/novelai/wallhaven 共用，智绘姬不受影响）
  $container.find('#gal-imagegen-cgmode').on('change', async function () {
    settings.imageGenCgMode = $(this).is(':checked');
    saveSettings();
    // 大香蕉的 CG 尺寸/人物外观区仅在剧情CG模式下有意义
    $container.find('#gal-banana-size-section, #gal-banana-appearance-section').toggle(settings.imageGenCgMode);
    if (getIsEnabled()) await injectCOTToWorldbook();
    showToast(settings.imageGenCgMode ? '生成剧情CG已开启（含人物）' : '已切换为纯场景背景生成');
  });
