...
export async function showSettingsPanel(topTab, subTab) {
  ...
  $('#gal-gpt-sovits-text-lang').on('change', function () { settings.gptSoVits = settings.gptSoVits || {}; settings.gptSoVits.textLang = $(this).val().trim() || 'auto'; saveSettings(); });
  $('#gal-gpt-sovits-split').on('change', function () { settings.gptSoVits = settings.gptSoVits || {}; settings.gptSoVits.textSplitMethod = $(this).val().trim() || 'cut5'; saveSettings(); });
  $('#gal-gpt-sovits-media-type').on('change', function () { settings.gptSoVits = settings.gptSoVits || {}; settings.gptSoVits.mediaType = $(this).val(); saveSettings(); });
  $('#gal-gpt-sovits-streaming').on('change', function () { settings.gptSoVits = settings.gptSoVits || {}; settings.gptSoVits.streamingMode = $(this).is(':checked'); saveSettings(); });
