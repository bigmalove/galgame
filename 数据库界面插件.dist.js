  ...
  function initEnhancedModeListener() {
    ...
    if (typeof eventOn === 'function' && typeof tavern_events !== 'undefined' && tavern_events.GENERATION_ENDED) {
      eventOn(tavern_events.GENERATION_ENDED, async messageId => {
        ...
        // 检查是否启用加强模式
        if (!settings.enhancedMode?.enabled) {
          console.log(`[${SCRIPT_NAME}] 加强模式: 未启用，跳过`);
          return;
        }
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .gal-gen-indicator {
        ...
        box-shadow:
          0 0 2.5rem rgba(0, 210, 255, 0.2),
          inset 0 0 2.5rem rgba(0, 210, 255, 0.05);
      ...
      @keyframes galGenPulse {
        0%, 100% {
          box-shadow:
            0 0 2.5rem rgba(0, 210, 255, 0.2),
            0 0 5rem rgba(0, 210, 255, 0.1),
            inset 0 0 2.5rem rgba(0, 210, 255, 0.05);
        }
        50% {
          box-shadow:
            0 0 3.75rem rgba(0, 210, 255, 0.4),
            0 0 7.5rem rgba(0, 210, 255, 0.2),
            inset 0 0 3.75rem rgba(0, 210, 255, 0.1);
        }
      }
      ...
      @keyframes galGenDot {
        ...
        40% { transform: scale(1); opacity: 1; box-shadow: 0 0 0.625rem #00d2ff; }
      }
  ...
  function setupGlobalEventListeners() {
    ...
    $(doc).on('click', '#gal-global-overlay [data-action="close-mode"]', function (e) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        // 3. 世界书逻辑已改为按需附加，不再全局启用/禁用
        console.log(`[${SCRIPT_NAME}] Galgame模式关闭（世界书按需附加）`);
  ...
  function showSettingsPanel() {
    ...
    `;
      ...
      $('#gal-enhanced-view-prompts').on('click', function () {
        ...
        $(topWindow.document.body).append(modalHtml);
        // 事件绑定
        const $modal = $(topWindow.document).find('#gal-prompts-modal');
