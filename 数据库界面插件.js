...
- **使用规则**:
  ...
  - **真实曲名**: AI必须根据知识库中真实存在的、适合当前场景的BGM，**直接输入具体的歌曲名称**。
  - **时机**:
    - 场景切换时。
    - 剧情发生重大转折时。
    - 情感基调剧烈变化时。
...
`;
  ...
  // 保存立绘
  async function saveSprite(characterId, expression, imageBlob, imageUrl = null) {
    if (!db) await initDB();
  ...
  // 保存背景图片
  async function saveBackground(sceneName, imageBlob, imageUrl = null) {
    if (!db) await initDB();
...
`;
  ...
  // 导入 JSON 资源配置
  async function importAssetsFromJson(file) {
    try {
      ...
      if (json.sprites) {
        for (const s of json.sprites) {
          if (s.characterId && s.expression && s.url) {
            await saveSprite(s.characterId, s.expression, null, s.url);
            count++;
          }
        }
      }
      ...
      if (json.backgrounds) {
        for (const bg of json.backgrounds) {
          if (bg.sceneName && bg.url) {
            await saveBackground(bg.sceneName, null, bg.url);
            count++;
          }
        }
      }
  ...
  const AssetIO = {
    ...
    // 导出所有资源为 ZIP
    async exportAllAssets(remoteBaseUrl = null) {
      try {
        showToast('正在准备导出...');
  ...
  async function showAssetManagerModal(activeTab = 'sprites') {
          ...
          <div style="padding: 20px 25px 15px; border-bottom: 1px solid #e0e0e0; flex-shrink: 0;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <div style="display: flex; gap: 8px;">
                ...
                <button class="gal-action-btn" id="gal-asset-import-json-btn" title="导入远程链接JSON" style="padding: 6px 12px; font-size: 0.9rem; background: #007bff; color: #fff; border-color: #007bff;">
                  <i class="fa-solid fa-link"></i> JSON
                </button>
    ...
    // 导入 JSON 按钮
    $('#gal-asset-import-json-btn').on('click', () => $('#gal-asset-import-json-input').click());
