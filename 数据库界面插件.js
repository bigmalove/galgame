...
* 这不仅增加了真实感，还为高裸露度提供了合理的“逻辑借口”。`,
  ...
  function _cleanVoiceNameFromWeightsBase(baseName) {
    ...
    // 去掉常见后缀（如 _v2pp / _v2proplus / _v3 等）
    const suffixes = [
  ...
  function _pickBestAudioFile(audioFiles) {
    ...
    const candidates = audioFiles.map(f => {
      const base = _stripFileExt(f.name);
      ...
      return { file: f, promptText, score: extScore * 1000 + lenScore };
    });
...
这是格式化任务，不是创作任务。你收到的文本已经是完整的，不需要也不允许继续写下去。`;
  ...
  const TTSManager = {
    ...
    _buildGptSoVitsApiUrl(pathname, queryParams = {}) {
      ...
      try {
        ...
        console.warn(`[${SCRIPT_NAME}] GPT-SoVITS: 构造API URL失败`, e);
        return '';
      }
    },
    ...
    async _setGptSoVitsWeights(kind, weightsPath) {
      ...
      try {
        ...
        // 兼容：代理/服务端可能返回纯文本 "success"
        const trimmed = String(text || '').trim();
        if (trimmed && trimmed.toLowerCase() !== 'success') {
          // 不强制报错：只记录，避免因非标准返回阻断播放
          console.warn(`[${SCRIPT_NAME}] GPT-SoVITS: set weights 返回非标准响应:`, trimmed);
        }
      }
      return true;
    },
  ...
  const SpriteManager = {
    ...
    rehydrateDisconnectedCharacters($overlay, preferredCharacterId = null, renderToken = null) {
      return __awaiter(this, void 0, void 0, function* () {
    ...
    // 更新立绘显示
    updateSprite($overlay, characterId, expression, renderToken = null) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        yield this.rehydrateDisconnectedCharacters($overlay, characterId, renderToken);
        ...
        if (this.activeCharacters.has(characterId)) {
          ...
          if (!isElementValid) {
            ...
            yield this.updateCharacterSprite($overlay, characterId, expression, spriteUrl, slot, false, renderToken);
          ...
          this.slotOwners.set(slot, characterId);
          yield this.updateCharacterSprite($overlay, characterId, expression, spriteUrl, slot, true, renderToken);
        }
        // 设置当前说话者
    ...
    // 更新/创建角色立绘元素
    updateCharacterSprite($overlay, characterId, expression, spriteUrl, slot, isEntering, renderToken = null) {
      return __awaiter(this, void 0, void 0, function* () {
  ...
  function scheduleOverlaySegmentDisplay(state, source = 'unknown') {
    ...
    return queueOverlayUpdate(source, () => updateOverlaySegmentDisplay(state, token));
  }
  /**
   * 创建或获取全局Galgame覆盖层
   */
   ...
   * 更新覆盖层的段落显示
   */
  function updateOverlaySegmentDisplay(state, expectedRenderToken = null) {
    return __awaiter(this, void 0, void 0, function* () {
      var _a, _b;
  ...
  function updateSegmentDisplay($container, state) {
    return __awaiter(this, void 0, void 0, function* () {
      yield scheduleOverlaySegmentDisplay(state, 'compat-updateSegmentDisplay');
    });
  }
  // ============================================
  // 交互功能
