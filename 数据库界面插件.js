  ...
  function parseGalgameContent(html) {
    ...
    if (maintextMatch) {
      ...
      if (maintextStart) {
        ...
        // ★ 修复：没有 <maintext> 标签时，只保留从第一个 <p> 标签开始的内容
        // 这样可以屏蔽掉 <p> 标签之前的思考链等内容
        const firstPIndex = cleanedHtml.search(/<p[\s>]/i);
        if (firstPIndex !== -1) {
          content = cleanedHtml.substring(firstPIndex);
          console.log(`[${SCRIPT_NAME}] [DEBUG] 截取从第一个 <p> 标签开始的内容，忽略前 ${firstPIndex} 字符`);
        }
        // 如果没有 <p> 标签，保持 content = cleanedHtml，让后续逻辑处理
      }
    }
    // 解析背景标签 - 使用预编译正则
    ...
    // ★ 修复：如果没有 <p> 标签，只有当存在其他 Galgame 标签时才回退解析纯文本
    // 这样可以避免把 AI 思考链等非 Galgame 格式内容显示在对话框中
    if (result.segments.length === 0) {
      // 检查是否有其他 Galgame 标签（表明这是一个有效的 Galgame 格式消息）
      const hasGalTags = result.currentBackground || result.bgm || result.options.length > 0;
      if (hasGalTags) {
      ...
      // 如果没有任何 Galgame 标签，保持 segments 为空，不显示任何内容
    }
    // ★ 新增逻辑：如果段落过长，进行切分
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .gal-realtime-toggle-wrapper {
        ...
        gap: 10px;
        padding: 6px 14px;
        background: linear-gradient(135deg, rgba(0, 210, 255, 0.12) 0%, rgba(255, 0, 85, 0.08) 100%);
        border-radius: 30px;
        border: 1px solid rgba(0, 210, 255, 0.3);
        transition: all 0.3s ease;
      }

      .gal-realtime-toggle-wrapper:hover {
        background: linear-gradient(135deg, rgba(0, 210, 255, 0.2) 0%, rgba(255, 0, 85, 0.12) 100%);
...
`;
  ...
  function updateOverlaySegmentDisplay(state) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const letment = state.segments[currentIndex];
      if (!segment) return;
  ...
  function setupMessageObserver() {
    ...
    chatObserver.obslete(chatContainer, { childList: true, subtree: false });
    // 为已存在的消息也设置内容监听并注入按钮
