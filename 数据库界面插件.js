...
(function () {
  ...
  // TTS启用状态存储键
  const TTS_ENABLED_KEY = `${SCRIPT_ID}_tts_enabled`;
...
这是格式化任务，不是创作任务。你收到的文本已经是完整的，不需要也不允许继续写下去。`,
  ...
  const enhancedModeState = {
    ...
    isSecondGeneration: false, // 标记是否正在进行第二次生成，防止循环触发
    // 保存最后一次第二次生成使用的完整提示词
    lastPrompts: null, // { systemPrompt, userPrompt, firstResult, timestamp }
  };
  // 当前设置 (全局)
